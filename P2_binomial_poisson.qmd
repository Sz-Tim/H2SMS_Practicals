...
It is much easier, of course, to do this using functions in R.
...
dbinom(0:3, num_trials, p_female)
sum(dbinom(0:3, num_trials, p_female))
```

```{r}
pbinom(3, num_trials, p_female)
```
...
The following code aims to calculate the cumulative probability for each value up to `num_female-1` (i.e., P(<1), P(<2), P(<3), and P(<4)) using `pbinom()`. 

```{r}
pbinom(0:num_female-1, num_trials, p_female)
```

It has a bug.

Identify the issue and correct it. Why was the first cumulative probability zero?

::: {.content-visible when-profile="key"}

::: callout-important

Because of R's order of operations, brackets are needed to first calculate `num_female-1`. 

When debugging, it is often useful to highlight *parts* of the code and run them. For example, if you highlight the first argument and run it, the issue is clear:

```{r}
0:num_female-1
```

R is creating a vector `0:num_female` and then subtracting `1` from each element. It is not possible to have `-1` females, and so R (correctly) tells you that the probability is 0.
