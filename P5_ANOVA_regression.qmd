...
ggplot(algae_df, aes(Treatment)) + 
  ...
  geom_point(data=algae_emm_df, aes(y=emmean), shape=1, size=3, colour="red")
```
...
Import the 'PhosphateCalibration' sheet from "H2SMS_practicalData.xlsx" (1st year practical data) into R and perform an appropriate analysis with absorbance predicted by concentration. Perform all appropriate checks for data quality and assumptions. Report relevant results with 92% confidence intervals.
...
```{r}
#| code-fold: true
#| code-summary: "Response variable and type"

# 'Absorbance' is continuous
```

```{r}
#| code-fold: true
#| code-summary: "Predictor variable and type"
...
```{r}
#| code-fold: true
#| code-summary: "Analysis type"

# Linear regression: The predictor and response are both continuous
```
...
Import the 'LimpetDist' sheet from "H2SMS_practicalData.xlsx", which gives travel distance of limpets on three different surface types. Perform an appropriate analysis of distance predicted by surface type. Make sure you perform appropriate data quality checks and assumption checks. Report results as appropriate.
...
# ANOVA: The predictor is categorical and the response is continuous
```
