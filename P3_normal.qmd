...
For next year, probably more intuitive to plot distributions instead. Maybe. Strange to go from a normal distribution to a skewed distribution...
...
ggplot(positive_df, aes(orig, new_value)) + 
  ...
  labs(title="Positive values", x="Original value", y="Transformed value")
```
...
ggplot(proportion_df, aes(orig, new_value)) + 
  geom_line() + 
  facet_wrap(~transformation, scales="free", nrow=1) + 
  labs(title="Proportions", x="Original value", y="Transformed value")
```
